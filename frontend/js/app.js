...
function deduplicarElencoLocal(jogadores) {
  const mapa = new Map();
  (jogadores || []).forEach((j) => {
    if (!j?.nome) return;
    const nome = String(j.nome).trim();
    if (!nome) return;
    const goleiro = !!j.goleiro;
    const chave = `${nome.toLowerCase()}|${goleiro}`;
    mapa.set(chave, {
      nome,
      estrelas: goleiro ? 0 : Number(j.estrelas) || 3,
      goleiro,
      apto: j.apto !== false,
    });
  });
...
/** Mantém inaptos do backup quando o servidor ainda não trouxe o campo certo. */
function mesclarAptoDoBackup(lista, backup) {
  if (!lista?.length) return backup || [];
  if (!backup?.length) return lista;
  const mapa = new Map(
    backup.map((b) => [`${String(b.nome).trim().toLowerCase()}|${!!b.goleiro}`, b.apto !== false])
  );
  return lista.map((j) => {
    if (j.apto === false) return j;
    const chave = `${String(j.nome).trim().toLowerCase()}|${!!j.goleiro}`;
    if (mapa.has(chave) && mapa.get(chave) === false) {
      return { ...j, apto: false };
    }
    return j;
  });
...
function elencoDoPayload(payload) {
  return deduplicarElencoLocal(
    (payload?.jogadores || []).map((j) => ({
      nome: j.nome,
...
/** Envia o elenco da conta (substitui o anterior no servidor). */
async function enviarElencoConta(jogadores) {
  const itens = deduplicarElencoLocal(jogadores).map((j) => ({
    nome: j.nome,
    estrelas: j.goleiro ? 0 : Number(j.estrelas) || 3,
    goleiro: !!j.goleiro,
    apto: j.apto !== false,
...
/** Sync do elenco só no sorteio — não a cada edição de jogador. */
async function sincronizarElencoNoSorteio() {
  if (!getToken()) return;
  try {
    await enviarElencoConta(LocalJogo.listarJogadores());
  } catch (_) {
...
/** Flag: usuário encerrou e Continuar deve sumir até criar/jogar de novo. */
const OCULTAR_CONTINUAR_KEY = "pelada_ocultar_continuar";
...
function marcarOcultarContinuar() {
  localStorage.setItem(OCULTAR_CONTINUAR_KEY, "1");
}

function limparOcultarContinuar() {
  localStorage.removeItem(OCULTAR_CONTINUAR_KEY);
}

function deveOcultarBoxContinuar() {
  return localStorage.getItem(OCULTAR_CONTINUAR_KEY) === "1" || !!lerSyncEncerrarPendente()?.payload;
}
...
function deveOcultarContinuar(peladaId) {
  if (deveOcultarBoxContinuar()) return true;
...
async function sincronizarEncerrarPendente() {
  ...
  try {
    ...
    const msg = String(err?.message || "").toLowerCase();
    // Já encerrou no servidor: tira da fila e garante elenco único na conta
    ...
    // Sync completo falhou: ainda assim tenta gravar só o elenco (uma vez)
    try {
      await enviarElencoConta(elencoDoPayload(item.payload));
    } catch (_) {
      /* fica na fila */
...
function cronoEstadoVazio(duracao) {
  ...
  return { segundosBase: d, inicioMs: null, rodando: false, duracao: d };
}

/** Dois relógios independentes: [0] de baixo, [1] de cima. */
const cronos = [cronoEstadoVazio(CRONO_SEGUNDOS_PADRAO), cronoEstadoVazio(CRONO_SEGUNDOS_PADRAO)];
let cronoRaf = null;
...
function cronoEls(idx) {
  if (idx === 1) {
    return {
      box: document.getElementById("cronometro-partida-2"),
      ring: document.getElementById("crono-ring-fg-2"),
      tempo: document.getElementById("cronometro-tempo-2"),
      btn: document.getElementById("btn-crono-toggle-2"),
    };
  }
  return {
    box: document.getElementById("cronometro-partida"),
    ring: document.getElementById("crono-ring-fg"),
    tempo: document.getElementById("cronometro-tempo"),
    btn: document.getElementById("btn-crono-toggle"),
  };
}

function cronoSegundosAgora(idx) {
  const c = cronos[idx];
  if (!c.rodando || c.inicioMs == null) return c.segundosBase;
  return c.segundosBase - (Date.now() - c.inicioMs) / 1000;
}

function algumCronoRodando() {
  return cronos.some((c) => c.rodando);
}
...
function progressoCrono(segundosFrac, duracao) {
  const d = duracao > 0 ? duracao : CRONO_SEGUNDOS_PADRAO;
  const decorrido = d - segundosFrac;
  return Math.min(1, Math.max(0, decorrido / d));
}

function atualizarAnelCrono(idx, segundosFrac) {
...
function atualizarUmCronoNaTela(idx) {
  const { tempo, btn } = cronoEls(idx);
  if (!tempo) return;

  const agora = cronoSegundosAgora(idx);
  const rodando = cronos[idx].rodando;
  tempo.textContent = formatarCrono(agora);
  tempo.classList.toggle("crono-zerado", agora <= 0);
  atualizarAnelCrono(idx, agora);

  if (btn) {
    const play = btn.querySelector(".crono-ico-play");
    const pause = btn.querySelector(".crono-ico-pause");
    if (play && pause) {
      play.classList.toggle("oculto", rodando);
      pause.classList.toggle("oculto", !rodando);
    }
    btn.setAttribute("aria-label", rodando ? "Pausar" : "Iniciar ou retomar");
  }
}
...
function pararLoopSeNinguemRodando() {
  if (!algumCronoRodando() && cronoRaf != null) {
    cancelAnimationFrame(cronoRaf);
    cronoRaf = null;
  }
}

function garantirLoopCrono() {
  if (!algumCronoRodando()) return;
  if (cronoRaf != null) cancelAnimationFrame(cronoRaf);
  cronoRaf = requestAnimationFrame(loopCrono);
}

function pausarCrono(idx) {
  const c = cronos[idx];
  if (c.rodando) c.segundosBase = cronoSegundosAgora(idx);
  c.rodando = false;
  c.inicioMs = null;
  pararLoopSeNinguemRodando();
}

function loopCrono() {
  atualizarCronoNaTela();
  if (algumCronoRodando()) {
    cronoRaf = requestAnimationFrame(loopCrono);
  } else {
    cronoRaf = null;
  }
...
function iniciarOuPausarCrono(idx) {
  const i = idx === 1 ? 1 : 0;
  if (cronos[i].rodando) {
    pausarCrono(i);
    atualizarCronoNaTela();
  ...
  cronos[i].inicioMs = Date.now();
  cronos[i].rodando = true;
  atualizarCronoNaTela();
  garantirLoopCrono();
  persistirCronometro(true);
}

function reiniciarCronometro(idx) {
  const i = idx === 1 ? 1 : 0;
  pausarCrono(i);
  const d = cronoDuracaoSegundos(i);
  cronos[i].duracao = d;
  cronos[i].segundosBase = d;
  atualizarCronoNaTela();
  persistirCronometro(true);
}

function reiniciarTodosCronometros() {
  for (let i = 0; i < 2; i++) {
    pausarCrono(i);
    const d = cronoDuracaoSegundos(i);
    cronos[i].duracao = d;
...
function aplicarDuracoesDasPrefs() {
  for (let i = 0; i < 2; i++) {
    const nova = cronoDuracaoSegundos(i);
    const c = cronos[i];
    const paradoNoInicio = !c.rodando && Math.abs(c.segundosBase - c.duracao) < 0.05;
    c.duracao = nova;
    if (paradoNoInicio) c.segundosBase = nova;
  }
  atualizarCronoNaTela();
  persistirCronometro(true);
}

function restaurarUmCrono(idx, salvo) {
  if (!salvo) {
    const d = cronoDuracaoSegundos(idx);
    cronos[idx] = cronoEstadoVazio(d);
    return;
  }
  let duracao = Number(salvo.duracao);
  if (!Number.isFinite(duracao) || duracao <= 0) duracao = cronoDuracaoSegundos(idx);
  let segundosBase = Number(salvo.segundosBase);
  if (!Number.isFinite(segundosBase)) segundosBase = duracao;
  const estavaRodando = !!salvo.rodando && salvo.inicioMs != null;
  cronos[idx] = {
    segundosBase,
    duracao,
    inicioMs: estavaRodando ? Number(salvo.inicioMs) : null,
...
function sincronizarCronoComPartida(partida) {
  ...
  if (salvo && String(salvo.partidaId) === id) {
    if (Array.isArray(salvo.timers) && salvo.timers.length) {
      restaurarUmCrono(0, salvo.timers[0]);
      restaurarUmCrono(1, salvo.timers[1] || null);
    } else {
      restaurarUmCrono(0, salvo);
      restaurarUmCrono(1, null);
    }
    atualizarCronoNaTela();
    garantirLoopCrono();
    return;
  ...
  reiniciarTodosCronometros();
}

function limparCronoDaPartida() {
  for (let i = 0; i < 2; i++) pausarCrono(i);
  const d0 = cronoDuracaoSegundos(0);
  const d1 = cronoDuracaoSegundos(1);
...
async function encerrarPelada() {
  ...
  try {
    await enviarElencoConta(elencoDoPayload(payload));
  } catch (_) {
    /* sync completo ainda tenta; backup local já ficou */
  }
  ...
  // Fecha qualquer outra pelada ainda "ativa" na conta (fantasma de Continuar)
  await encerrarResiduosAtivosNoServidor().catch(() => {});
  if (!enviou) {
...
async function entrarNaHome() {
  ...
  try {
    ...
    if (ativa && ativa.id && deveOcultarContinuar(ativa.id)) {
      // Já encerrada neste aparelho — não mostra Continuar
      estado.peladaAtiva = null;
      idAtiva = null;
      await encerrarResiduosAtivosNoServidor().catch(() => {});
      ...
      idAtiva = ativa.id;
      if (box && !deveOcultarBoxContinuar()) {
      ...
      estado.peladaAtiva = null;
      if (!lerSyncEncerrarPendente()?.payload) limparOcultarContinuar();
    }
  } catch (_) {
    /* API oscilando */
  }

  // Jogo local só mostra Continuar se o usuário não acabou de encerrar
  if (temLocal && box && !deveOcultarBoxContinuar()) {
    limparOcultarContinuar();
    box.classList.remove("oculto");
    document.querySelector("#box-continuar .continuar-txt").textContent =
  ...
  try {
    await comLoading(async () => {
      ...
      if (todos.length) {
        toast(
          inaptos
            ? `Elenco carregado: ${todos.length} · ${inaptos} inapto(s)`
            : `Elenco carregado: ${todos.length} pessoa(s)`
        );
      } else {
        toast("Pelada criada — cadastre os jogadores e sorteie os times");
      }
    }, "Criando pelada...");
  } catch (err) {
    toast(err.message);
